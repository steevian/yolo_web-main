  ...
  for (let k in opt) {
    ...
    if (matches) {
      matches.forEach(match => {
        // 如果匹配到的字符串长度大于1，需要补零
        const value = opt[k];
        const paddedValue = match.length > 1 ? value.padStart(match.length, '0') : value;
        format = format.replace(match, paddedValue);
      });
    }
  }
 ...
 * @returns 格式化后的字符串
 */
export function formatDateSimple(date: Date, format: string = 'YYYY-MM-DD HH:mm:ss'): string {
